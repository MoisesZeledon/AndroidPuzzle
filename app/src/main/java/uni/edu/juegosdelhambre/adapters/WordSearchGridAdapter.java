package uni.edu.juegosdelhambre.adapters;
...
import uni.edu.juegosdelhambre.utils.DeviceUtils;
...
public class WordSearchGridAdapter extends BaseAdapter {
    ...
    public View getView(final int pos, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            // ToDo: Check any conflict Here
            int difference = ((WordSearchManager.HARD_MAX_WORDLENGTH + WordSearchManager.HARD_MAX_DIMENSION_OFFSET - WordSearchManager.EASY_MIN_WORDLENGTH) / 6);
        ...
        if (DISPLAY_TYPE.equals(DisplayType.STANDARD_BLUE)) {
            color = mContext.getResources().getColor(R.color.md_light_blue_A400);
        } else {
            if ((pos / mWordSearchDimension) % 2 == 0)
                color = mContext.getResources().getColor(pos % 2 == 0 ? R.color.md_light_blue_A400 : R.color.green);
            else
                color = mContext.getResources().getColor((pos - mWordSearchDimension) % 2 == 0 ? R.color.green : R.color.md_light_blue_A400);
        }
